...
class DBConnector:
    ...
    def get_all_user_list(self, prefix):
        self.cursor.execute("select ifnull(user_id, ''), ifnull(email_address, ''), ifnull(email_id, ''), ifnull(email_host, ''), ifnull(user_name, ''), ifnull(nick_name, ''), ifnull(homepage, ''), ifnull(blog, ''), ifnull(birthday, ''), ifnull(allow_mailing, ''), ifnull(allow_message, ''), ifnull(is_admin, ''), ifnull(denied, ''), ifnull(limit_date, ''), ifnull(regdate, ''), ifnull(last_login, ''), ifnull(change_password_date, ''), ifnull(description, ''), ifnull(extra_vars, '') from {0}_member;".format(prefix))
        return self.cursor.fetchall()
    def get_group_user_list(self, prefix, group_srl):
        self.cursor.execute("select ifnull(user_id, ''), ifnull(email_address, ''), ifnull(email_id, ''), ifnull(email_host, ''), ifnull(user_name, ''), ifnull(nick_name, ''), ifnull(homepage, ''), ifnull(blog, ''), ifnull(birthday, ''), ifnull(allow_mailing, ''), ifnull(allow_message, ''), ifnull(is_admin, ''), ifnull(denied, ''), ifnull(limit_date, ''), ifnull({0}_member.regdate, ''), ifnull(last_login, ''), ifnull(change_password_date, ''), ifnull(description, ''), ifnull(extra_vars, '') from {0}_member, {0}_member_group_member where {0}_member.member_srl = {0}_member_group_member.member_srl and {0}_member_group_member.group_srl = {1};".format(prefix, group_srl))
        return self.cursor.fetchall()
